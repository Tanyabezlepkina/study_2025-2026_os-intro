...
На рисунке 1 представлен листинг командного файла search.sh. В нём используются конструкции getopts для разбора ключей -i, -o, -p, -C, -n, а также условный оператор if для проверки существования входного файла и перенаправление вывода grep. Скрипт реализует поиск строк по шаблону с возможностью учёта регистра и вывода номеров строк (рис. -@fig:001)
![код листинга командного файла search.sh](image/1.png){#fig:001 width=50%}

На рисунке 2 показан процесс выполнения скрипта. Командой chmod +x search.sh заданы права на выполнение. Затем скрипт запущен с ключами: -i test.txt -p "Hi" -C -n -o result.txt. В результате в файл result.txt выведены строки, содержащие "Hi" без учёта регистра, с номерами строк (рис. -@fig:002)
![процесс выполнения скрипта для 1 задания](image/2.png){#fig:002 width=50%}

Рисунок 3 — листинг checknum.cpp. Программа вводит число и завершается с exit(1) если >0, exit(2) если <0, exit(0) если =0, exit(255) при ошибке ввода (рис. -@fig:003)
![листинг checknum.cpp.](image/3.png){#fig:003 width=50%}

Рисунок 4 — листинг run_check.sh. Скрипт запускает checknum, анализирует $? через case и выводит сообщение (рис. -@fig:004)
![листинг run_check.sh.](image/4.png){#fig:004 width=50%}

Рисунок 5 — выполнение. Компиляция g++ checknum.cpp -o checknum, запуск run_check.sh. При вводе 5 — «больше нуля», -2 — «меньше нуля», 0 — «равно нулю», abc — «ошибка». (рис. -@fig:005)
![процесс выполнения скрипта для 2 задания](image/5.png){#fig:005 width=50%}

Рисунок 6 — листинг files.sh. Скрипт создаёт файлы 1.tmp … N.tmp через цикл for, если второй аргумент delete — удаляет. (рис. -@fig:006)
![листинг files.sh.](image/6.png){#fig:006 width=50%}

Рисунок 7 — выполнение. ./files.sh 5 — созданы пять файлов, ls проверяет. ./files.sh 5 delete — файлы удалены. (рис. -@fig:007)
![процесс выполнения скрипта для 3 задания](image/7.png){#fig:007 width=50%}

Рисунок 8 — листинг backup.sh. Скрипт переходит в каталог, find -mtime -7 ищет файлы новее 7 дней, tar создаёт архив. (рис. -@fig:008)
![листинг backup.sh.](image/8.png){#fig:008 width=50%}

Рисунок 8 — листинг backup.sh. Скрипт переходит в каталог, find -mtime -7 ищет файлы новее 7 дней, tar создаёт архив. (рис. -@fig:009)
